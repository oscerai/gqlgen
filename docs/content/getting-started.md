...
```shell
go get -d github.com/oscerai/gqlgen@VERSION
```
...
And run `go run github.com/oscerai/gqlgen generate`.

>
> If you run into this error `package github.com/oscerai/gqlgen: no Go files` while executing the `generate` command above, follow the instructions in [this](https://github.com/oscerai/gqlgen/issues/800#issuecomment-888908950) comment for a possible solution.
...
```go
//go:generate go run github.com/oscerai/gqlgen generate
```
